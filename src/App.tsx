...
import { add as tAdd, remove as tRemove, updateState, updateInputHead, updateStack, updateInput } from './lib/transitions';
...
function App() {
  const [defaultTraversal, setDefaultTraversal] = useState<Traversal>({id: 0, history: [-1], stateId: 0, transitionId: -1, stack: ["Z", "Z"], inputHead: 0, end: 0})
  ...
  const [stacks, setStacks] = useState<string[]>(["Z", "Z"])
  ...
  const clientUpdateCInput = (transitionId: number, cInput: string) => {
    let newTransitions = updateInput(transitionId, cInput, [...transitions])
    setTransitions(newTransitions)
  }

  const clientUpdateCState = (transitionId: number, cStateName: string) => {
    let newPA = updateState(transitionId, cStateName, true, [...transitions], [...states], [...connections])
    setTransitions(newPA.transitions)
    setStates(newPA.states)
    setConnections(newPA.connections)
  }
  ...
  const clientUpdateNState = (transitionId: number, cStateName: string) => {
    let newPA = updateState(transitionId, cStateName, false, [...transitions], [...states], [...connections])
    setTransitions(newPA.transitions)
    setStates(newPA.states)
    setConnections(newPA.connections)
  }
  ...
  const clientUpdateCStack = (transitionId: number, value: string, stackIndex: number) => {
    let newTransitions = updateStack(transitionId, value, true, stackIndex, [...transitions])
    setTransitions(newTransitions)
  }

  const clientUpdateNStack = (transitionId: number, value: string, stackIndex: number) => {
    let newTransitions = updateStack(transitionId, value, false, stackIndex, [...transitions])
    setTransitions(newTransitions)
  }

  const clientUpdateInputHead = (transitionId: number, value: number) => {
    let newTransitions = updateInputHead(transitionId, value, [...transitions])
    setTransitions(newTransitions)
  }
  ...
  const clientAddGuiTransition = (cState: State, nState: State) => {
    let newPA = tAdd([...transitions], [...states], stackCountMax, [...connections])
    newPA = updateState(newPA.transitions[newPA.transitions.length - 1].id, cState.name, true, [...newPA.transitions], [...newPA.states], [...newPA.connections])
    newPA = updateState(newPA.transitions[newPA.transitions.length - 1].id, nState.name, false, [...newPA.transitions], [...newPA.states], [...newPA.connections])
      ...
      <TxtWrapper
          ...
          onAddState={() => {clientAddState()}}/>
            ...
            <SimWrapper
              ...
              onStacksUpdate={(values: string[]) => {setStacks(values)}}
              onRun={() => {clientSimRun()}}
